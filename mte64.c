...
static void emit_ops_maybe_rol(int is_rotate) {
  ...
  if (is_rotate) {
    ...
    if ((DL & 8)) {
      ...
      // we're optimizing rotate ops from the table (the same is done for
      // small add/subs and xor -1)
    }
  }
